...
def upgrade() -> None:
    ...
    op.add_column('users', sa.Column('documents_analyzed_count', sa.Integer(), nullable=True, server_default='0'))
    ...
    result = connection.execute(sa.text("SELECT COUNT(*) FROM dvf_imports WHERE status = 'completed'")).fetchone()
    ...
    connection.execute(
        ...
        """),
        {"total_records": dvf_count, "total_imports": import_count, "last_updated": datetime.utcnow()}
    )
