...
def upgrade():
    op.add_column("photo_redesigns", sa.Column("redesign_uuid", sa.String(), nullable=True))
    connection = op.get_bind()
    result = connection.execute(sa.text("SELECT id FROM photo_redesigns"))
    for row in result:
        connection.execute(
            sa.text("UPDATE photo_redesigns SET redesign_uuid = :uuid WHERE id = :id"),
            {"uuid": str(uuid.uuid4()), "id": row[0]},
        )
    op.alter_column("photo_redesigns", "redesign_uuid", nullable=False)
    op.create_unique_constraint(
        "uq_photo_redesigns_redesign_uuid", "photo_redesigns", ["redesign_uuid"]
    )
    op.create_index(
        "ix_photo_redesigns_redesign_uuid", "photo_redesigns", ["redesign_uuid"], unique=False
    )


def downgrade():
    op.drop_index("ix_photo_redesigns_redesign_uuid", table_name="photo_redesigns")
    op.drop_constraint("uq_photo_redesigns_redesign_uuid", "photo_redesigns", type_="unique")
