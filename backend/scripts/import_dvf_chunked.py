...
class ChunkedDVFImporter:
    ...
    def clean_chunk(self, chunk_df: pd.DataFrame, data_year: int) -> pd.DataFrame:
        ...
        df = chunk_df.rename(
            columns={k: v for k, v in column_mapping.items() if k in chunk_df.columns}
        )
        ...
        if "sale_price" in df.columns:
            df["sale_price"] = (
                df["sale_price"].astype(str).str.replace(" ", "").str.replace(",", ".")
            )
        ...
        df = df.drop_duplicates(
            subset=["sale_date", "sale_price", "address", "property_type", "surface_area"]
        )
    ...
    def import_file(self, file_path: str, data_year: int, force: bool = False) -> DVFImport:
        ...
        if not force and self.is_already_imported(file_hash):
            ...
            existing = (
                self.db.query(DVFImport).filter(DVFImport.source_file_hash == file_hash).first()
            )
            return existing
        ...
        try:
            ...
            for chunk in csv_chunks:
                ...
                logger.info(
                    f"Processing chunk {chunk_num} ({len(chunk):,} raw rows, {total_raw_rows:,} total raw)"
                )
                ...
                logger.info(
                    f"  Cleaned to {len(cleaned_chunk):,} rows ({total_cleaned_rows:,} total cleaned)"
                )
            ...
            import_record.duration_seconds = (
            ...
            logger.info(
                f"  Throughput: {total_cleaned_rows / import_record.duration_seconds:.0f} records/sec"
            )
...
def main():
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force re-import even if file hash exists"
    )
    ...
    try:
        importer = ChunkedDVFImporter(
            db, read_chunk_size=args.read_chunk_size, write_batch_size=args.write_batch_size
        )
