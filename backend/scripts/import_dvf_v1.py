...
def clean_and_transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "street_number" in df.columns:
        ...
        print(
            f"After street_number to numeric: {len(df)} records, NaN count: {df['street_number'].isna().sum()}"
        )
    ...
    if "sale_date" in df.columns:
        ...
        print(
            f"After date conversion: {len(df)} records, NaN count: {df['sale_date'].isna().sum()}"
        )
    ...
    df = df.drop_duplicates(
        subset=["sale_date", "sale_price", "address", "property_type", "surface_area"], keep="first"
    )
